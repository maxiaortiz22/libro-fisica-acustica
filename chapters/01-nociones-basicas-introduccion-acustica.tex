...
\begin{figure}[htbp]
    \centering
    \input{figures/tikz/unidad-1/fuente-medio-receptor.tex}
...
La descripción matemática de la oscilación, la propagación y los parámetros de una onda se desarrolla en el capítulo~\ref{chap:unidad-3}. En esta unidad basta con conservar la idea central: una onda mecánica es una perturbación que se propaga en un medio, no un transporte neto de materia desde la fuente hasta el receptor.
...
\verify{El SI se organiza a partir de siete magnitudes fundamentales; las demás magnitudes se expresan mediante relaciones entre ellas.} La tabla~\ref{tab:u1-magnitudes-fundamentales} presenta las siete magnitudes fundamentales y sus unidades.
...
La expresión \(F=ma\) se utilizará como forma introductoria de la segunda ley de Newton para un cuerpo de masa constante y una fuerza neta. Sus condiciones y aplicaciones se desarrollan en el capítulo~\ref{chap:unidad-2}.
...
\verify{Como valor de referencia didáctico, para aire cercano a \(20\,^{\circ}\text{C}\) suele emplearse \(c\approx343\,\text{m}/\text{s}\).} Este valor no es universal: depende de las propiedades y del estado del medio.
...
\begin{figure}[htbp]
    \centering
    \input{figures/tikz/unidad-1/dependencias-dimensionales.tex}
...
\begin{figure}[htbp]
    \centering
    \input{figures/tikz/unidad-1/funcion-directa-inversa.tex}
...
El círculo trigonométrico permite extender seno y coseno más allá de los triángulos rectángulos y será útil para describir fenómenos periódicos. La ecuación completa de una oscilación sinusoidal, su amplitud, frecuencia y fase pertenecen al capítulo~\ref{chap:unidad-3}.
...
\begin{figure}[htbp]
    \centering
    \input{figures/tikz/unidad-1/circulo-trigonometrico.tex}
...
\verify{El decibel, de símbolo \(\text{dB}\), se utiliza para expresar el valor de una relación logarítmica especificada.} No es una unidad absoluta de presión, intensidad o potencia. Para interpretar un valor en decibeles se debe conocer qué magnitud se compara y cuál es la referencia.
...
\begin{figure}[htbp]
    \centering
    \input{figures/tikz/unidad-1/escala-lineal-logaritmica.tex}
...
La definición del nivel de presión sonora, el uso del factor \(20\), los valores eficaces y las referencias acústicas se desarrollan en el capítulo~\ref{chap:unidad-4}. También se distinguirán allí los niveles de presión, intensidad y potencia.
...
\bigskip
\noindent\emph{Conclusión.} Esta unidad estableció las herramientas necesarias para describir fenómenos sin confundir magnitudes, unidades y percepciones. En el capítulo~\ref{chap:unidad-2} se aplicarán masa, aceleración, fuerza y energía al estudio de sistemas mecánicos; en los capítulos~\ref{chap:unidad-3} y~\ref{chap:unidad-4} se desarrollarán, respectivamente, el formalismo ondulatorio y las magnitudes acústicas.
