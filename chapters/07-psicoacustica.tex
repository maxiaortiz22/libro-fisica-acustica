...
normal. La anatomía y la transducción se estudiaron en la Unidad 6. Las
alteraciones, los estudios auditivos y los dispositivos de rehabilitación
corresponden a la Unidad 8. La física de los recintos se profundizará en la
Unidad 9, y los tipos de ruido y la aplicación audiométrica del
enmascaramiento se tratarán en la Unidad 10.
...
El capítulo~\ref{chap:unidad-4} definió presión acústica, nivel de presión
sonora y campo libre. El capítulo~\ref{chap:unidad-5} diferenció frecuencia
fundamental, componentes espectrales, espectro de una señal y respuesta en
frecuencia de un sistema. El capítulo~\ref{chap:unidad-6} presentó la
...
\verify{Los resultados psicofísicos dependen del procedimiento, de las
características del estímulo, del ambiente, del estado del oyente y del
criterio adoptado para definir la respuesta.} Por eso, una curva o una cifra
...
\verify{En personas jóvenes otológicamente normales, la región de menor nivel
...
entre procedimientos.} Esta región de menor umbral se denomina aquí
...
utilizarse como actividad didáctica. \verify{La forma de ambas fronteras y el
intervalo de frecuencias representado dependen de la fuente y del criterio.}
...
\begin{figure}[htbp]
    \centering
    \input{figures/tikz/unidad-7/umbral-campo-audible.tex}
...
\verify{La transferencia campo--tímpano depende de la dirección de llegada,
...
sensibilidad de una región de frecuencias medias.} El modelo ideal de cuarto
...
\begin{figure}[htbp]
    \centering
    \input{figures/tikz/unidad-7/campo-cae-timpano.tex}
...
\verify{Las curvas normalizadas de igual sonoridad se obtienen para tonos
...
deben verificarse antes de incorporar una figura numérica.} Fuera de esas
...
\begin{figure}[htbp]
    \centering
    \input{figures/tikz/unidad-7/construccion-isofonica.tex}
...
\verify{La altura tonal puede depender de periodicidad temporal, contenido
...
en esa frecuencia no esté presente.} Este fenómeno de fundamental ausente
...
tonal, sonoridad y duración global. \verify{Participan la envolvente
espectral, la presencia de componentes armónicas o inarmónicas, el ruido, el
ataque, el decaimiento y otras variaciones temporales.} Por eso, el timbre se
...
\verify{Para estímulos breves, la detección y la sonoridad dependen de la
duración dentro de intervalos cuyo alcance varía con el estímulo y el
procedimiento.} No corresponde reemplazar esa dependencia por un único
...
(\(\text{phon}\)). \verify{Su valor corresponde al nivel de presión sonora de
un tono de referencia de \(1000\,\text{Hz}\) que se juzga igualmente sonoro
bajo condiciones especificadas.} No se debe escribir ``un fon es un dB SPL'':
...
(\(\text{sone}\)). \verify{La escala se define de modo que
\(1\,\text{sone}\) corresponda a \(40\,\text{phon}\) y una duplicación de
sonoridad corresponda a una duplicación del número de sones.}

\verify{Para una conversión introductoria con niveles de sonoridad no menores
que \(40\,\text{phon}\), puede usarse}
...
\begin{figure}[htbp]
    \centering
    \input{figures/tikz/unidad-7/fones-sones.tex}
...
\verify{El enmascaramiento suele ser mayor cuando ambas producen excitación en
...
superiores al aumentar el nivel.}
...
\verify{Para estimar la ERB de filtros auditivos de oyentes jóvenes con
...
\begin{figure}[htbp]
    \centering
    \input{figures/tikz/unidad-7/enmascaramiento-erb.tex}
...
otra, medido en segundos (\(\text{s}\)). \verify{La elevación del umbral suele
...
hacia atrás.} No corresponde asignar una única ``ventana temporal'' válida
...
\begin{figure}[htbp]
    \centering
    \input{figures/tikz/unidad-7/enmascaramiento-temporal.tex}
...
\verify{La semejanza entre voces, la incertidumbre sobre la fuente y la
...
componentes. La Unidad 10 describirá las propiedades físicas de ruidos
...
físicamente en la Unidad 9. Desde la percepción importa que parte de la energía
de un segmento persiste y se superpone con segmentos posteriores.

\verify{Una reverberación excesiva puede reducir contrastes temporales y
...
lingüístico y las características del oyente.} Por eso, no existe una ecuación
lineal universal que convierta SNR y tiempo de reverberación en porcentaje de
palabras comprendidas.

\verify{El índice de transmisión del habla (STI) y el índice de
inteligibilidad del habla (SII) son métodos diferentes, con entradas, supuestos
y dominios propios.} En esta unidad basta reconocer qué problema intentan
...
resultado de otra sala o población. \verify{Los modelos que estiman ALCons a
...
\verify{El \textbf{efecto de precedencia} reúne al menos tres observaciones
...
\verify{Al variar el retardo y el nivel de una reflexión pueden observarse
coloración, aumento de extensión o sonoridad, desplazamientos espaciales,
fusión y, para ciertas condiciones, una repetición separada o eco.} La
...
\begin{figure}[htbp]
    \centering
    \input{figures/tikz/unidad-7/precedencia-reflexiones.tex}
...
\verify{Para muchas señales, las ITD aportan información especialmente útil
...
que no existe una frontera única que separe ambos mecanismos.}
...
\verify{Distintas posiciones pueden producir ITD e ILD semejantes y formar un
...
arriba--abajo y adelante--atrás.} Por eso, la localización no puede explicarse
solo mediante dos números interaurales.

La figura~\ref{localizacion_fuentes} integra las pistas interaurales con las
pistas espectrales y dinámicas que reducen ambigüedades.

\begin{figure}[htbp]
    \centering
    \input{figures/tikz/unidad-7/audicion-espacial.tex}
...
\verify{Separar espacialmente la voz objetivo y las voces competidoras puede
producir liberación espacial del enmascaramiento. El beneficio depende de la
escena, del oyente y del acceso a las pistas binaurales.} Un micrófono
...
\begin{figure}[htbp]
    \centering
    \input{figures/tikz/unidad-7/fuentes-concurrentes.tex}
...
interpretación clínica pertenecen a la Unidad 8 y a la formación
fonoaudiológica específica. La Unidad 10 retomará el enmascaramiento con los
...
selectividad frecuencial; la Unidad 10 aplicará estos fundamentos a distintos
ruidos.
...
\begin{enumerate}[label=\textbf{I\arabic*.},leftmargin=*]
    ...
    \item En la figura~\ref{localizacion_fuentes}, identifique las pistas
...
\begin{enumerate}[label=\textbf{P\arabic*.},leftmargin=*]
    ...
    \end{enumerate}
    Indique qué contenidos corresponden a las Unidades 7, 9 y 10.
\end{enumerate}
...
\begin{enumerate}[label=\textbf{P\arabic*.},leftmargin=*]
    ...
    \begin{enumerate}
        ...
        definida.
    \end{enumerate}
    La Unidad 7 es propietaria de los mecanismos perceptuales; la Unidad 9
    desarrolla la propagación y la reverberación; la Unidad 10 caracteriza los
