...
energía introducidas en el capítulo~\ref{chap:unidad-2}. Del
capítulo~\ref{chap:unidad-3} se retomarán la onda viajera, la frecuencia, la
...
\verify{Desde el punto de vista físico, un sonido es una perturbación mecánica
que se propaga en un medio material y puede transportar energía sin producir
un transporte neto de materia.} El fenómeno puede existir aunque no haya un
...
\verify{En los sólidos también pueden propagarse modos transversales,
superficiales y combinaciones más complejas.} Por este motivo, no conviene
...
\verify{Para tonos simples y bajo condiciones comparables, la frecuencia suele
...
frecuencia, la duración, el contenido espectral, el oyente y el contexto.} Los
...
de él; cuando retrocede, favorece una rarefacción. El capítulo~\ref{chap:unidad-3}
...
cúbico (\(\text{kg}/\text{m}^3\)). \verify{En el modelo lineal de pequeñas
perturbaciones, estas magnitudes se relacionan mediante}
...
\verify{Para aire seco y temperaturas próximas a las ambientales puede usarse
como aproximación didáctica}

\begin{equation}
    c\approx331+0{,}6\theta,
    \label{eq:u4-velocidad-aire}
\end{equation}

donde \(\theta\) es la temperatura en grados Celsius (\(^{\circ}\text{C}\)) y
el resultado \(c\) se expresa en metros por segundo
(\(\text{m}/\text{s}\)). A \(20\,^{\circ}\text{C}\), por ejemplo,
\nopagebreak[4]

\[
    c\approx331+0{,}6(20)=343\,\text{m}/\text{s}.
\]

Este valor es una aproximación para las condiciones indicadas, no una constante
universal. Si una onda cambia de medio, la frecuencia impuesta por la fuente se
conserva en la interfaz dentro del modelo usual, mientras que la velocidad y
la longitud de onda pueden cambiar. La relación
\(\lambda=c/f\) se desarrolló en la
...
frecuencia, de la fase y del tipo de campo. \verify{Para una onda plana
progresiva en un fluido homogéneo y sin pérdidas, presión y velocidad están en
fase y su cociente es la impedancia característica \(Z_0\):}
...
\verify{Para incidencia normal, medios homogéneos y un modelo sin pérdidas, el
...
\verify{En esas mismas condiciones y para medios sin pérdidas, la fracción de
intensidad reflejada es \(R_I=|R_p|^2\).} Esta relación no debe confundirse con
...
\begin{figure}[htbp]
    \centering
    \input{figures/tikz/unidad-4/impedancia-reflexion-interfaz.tex}
...
\verify{En una onda plana progresiva sinusoidal, en un fluido sin pérdidas, la
intensidad media también puede calcularse mediante}
...
\begin{figure}[!htbp]
    \centering
    \input{figures/tikz/unidad-4/presion-velocidad-intensidad.tex}
...
\begin{figure}[!htbp]
    \centering
    \input{figures/tikz/unidad-4/rms-sinusoide.tex}
...
El capítulo~\ref{chap:unidad-3} definió el tono puro ideal como una señal
...
El cociente dentro del logaritmo es adimensional. \verify{Para sonidos en aire
...
sonora en \(\text{dB SPL}\).}

\verify{En acústica subacuática se utiliza una referencia de presión diferente,
habitualmente \(1\,\mu\text{Pa}\), que debe declararse de forma explícita.}
...
\begin{figure}[htbp]
    \centering
    \input{figures/tikz/unidad-4/escala-presion-db-spl.tex}
...
\verify{En acústica aérea se emplean comúnmente
\(I_\mathrm{ref}=10^{-12}\,\text{W}/\text{m}^2\) y
\(W_\mathrm{ref}=10^{-12}\,\text{W}\).} La referencia debe acompañar cualquier
...
\verify{El nivel de audición expresado en \(\text{dB HL}\) depende de
...
\verify{Las ponderaciones frecuenciales, como A, C y Z, se aplican en
mediciones electroacústicas con objetivos definidos y no son nombres
alternativos para \(\text{dB SPL}\) o \(\text{dB HL}\).} Su desarrollo se
...
El principio de superposición del capítulo~\ref{chap:unidad-3} establece que,
...
\begin{figure}[htbp]
    \centering
    \input{figures/tikz/unidad-4/suma-coherente-no-correlacionada.tex}
...
\verify{Un campo difuso ideal presenta, en promedio, flujo de energía
distribuido en todas las direcciones y propiedades estadísticas
aproximadamente uniformes en la región considerada.} Una sala ruidosa no es
necesariamente un campo difuso.

\verify{La audiometría en campo sonoro requiere condiciones geométricas,
electroacústicas y de calibración específicas; puede utilizar campos libre,
cuasilibre o difuso según el procedimiento adoptado.} El diseño y la
...
\verify{En la región de campo lejano de una onda esférica progresiva, y si la
impedancia del medio permanece constante, la presión eficaz es proporcional a
\(1/r\).} En esas condiciones, la diferencia de nivel de presión se escribe
...
\begin{figure}[htbp]
    \centering
    \input{figures/tikz/unidad-4/propagacion-esferica.tex}
...
El capítulo~\ref{chap:unidad-9} aplicará este modelo a condiciones ambientales,
...
dirección angular. \verify{En campo lejano, para una intensidad
\(I(r,\theta,\phi)\) medida a distancia \(r\) y una potencia total \(W\), el
factor puede expresarse como}
...
\begin{figure}[htbp]
    \centering
    \input{figures/tikz/unidad-4/directividad-q.tex}
...
\begin{itemize}
    ...
    \item \verify{En audiometría, la interpretación del nivel presentado
    depende del transductor, de la frecuencia y de referencias de
    calibración.} Por esta razón, una presión medida en un punto no debe
...
\begin{enumerate}[label=\textbf{N\arabic*.},leftmargin=*]
    ...
    \(\theta=15{,}0\,^{\circ}\text{C}\), utilice la aproximación
    \(c\approx331+0{,}6\theta\), con \(c\) en
    \(\text{m}/\text{s}\). Calcule \(c\) y la longitud de onda de un tono de
    frecuencia \(f=500\,\text{Hz}\).
