% !TeX root = ../main.tex
\chapter[Unidad 8: Enfermedades auditivas, diagnóstico y rehabilitación]{Unidad 8: Enfermedades auditivas, estudios diagnósticos y técnicas de rehabilitación}
\label{chap:unidad-8}
...
contenidos: alteraciones auditivas, estudios diagnósticos y dispositivos de
...
\begin{itemize}
    ...
    (Capítulo~\ref{chap:unidad-5});
    \item las vías aérea y ósea involucran mecanismos físicos diferentes
    (Capítulo~\ref{chap:unidad-6});
    \item frecuencia, altura tonal, presión sonora, sonoridad y nivel de
    audición no son términos intercambiables
    (Capítulo~\ref{chap:unidad-7}).
\end{itemize}

La Unidad 6 presentó el funcionamiento auditivo habitual y la Unidad 7
estudió relaciones psicofísicas. Aquí esos conceptos se aplican a
alteraciones, pruebas y dispositivos. La caracterización de fuentes de ruido
y el enmascaramiento ambiental se retoman con mayor detalle en la Unidad 10.
...
\verify{En la práctica audiológica se suele distinguir entre componentes
...
con una batería de pruebas, no inferirse de un único dato.}
...
\verify{La magnitud y el curso temporal de un TTS dependen, entre otros
...
una fuente primaria y no deben expresarse como intervalos universales.}
...
asociados con exposición a ruido. \verify{Su evaluación requiere integrar la
...
no demuestra por sí sola su etiología.}
...
\(L_{Aeq,1\,\mathrm{h}}=95\,\mathrm{dB(A)}\) y que su contribución durante las
...
exposición sea segura, legal o aceptable. \verify{La evaluación ocupacional
...
\verify{El \textbf{trauma acústico} se asocia con un episodio acústico abrupto
y de alta energía, por ejemplo una explosión.} \verify{Según las características
del evento, puede comprometer estructuras del oído externo, medio o interno;
el nivel de pico aislado no permite predecir de manera universal la lesión.}
...
Este capítulo no indica conductas terapéuticas. \verify{La aparición súbita de
síntomas auditivos o vestibulares después de un evento intenso requiere
evaluación sanitaria oportuna de acuerdo con protocolos clínicos vigentes.}

La \textbf{ototoxicidad} no es una propiedad acústica de un sonido.
\verify{Ciertos medicamentos y agentes químicos pueden afectar funciones
cocleares o vestibulares. El riesgo depende del agente, la dosis acumulada,
la vía de administración, factores individuales y exposiciones concurrentes.}
...
\verify{El \textbf{tinnitus} o \textbf{acúfeno} es una percepción sonora
referida sin que exista una fuente acústica externa correspondiente.}
\verify{La mayoría de
...
\verify{El tinnitus puede coexistir con umbrales audiométricos diversos y con
condiciones auditivas o no auditivas diferentes. Su evaluación clínica no
debe reducirse a una única correspondencia de frecuencia o nivel.}

\subsection{Presbiacusia}
\label{sec:u8-presbiacusia}

\verify{La \textbf{presbiacusia} designa cambios auditivos asociados con el
envejecimiento.} \verify{Es un proceso heterogéneo y multifactorial en el que
...
no existe una pendiente audiométrica única que defina todos los casos.}
...
\verify{Las pruebas \textbf{conductuales} requieren una acción de la persona,
...
registro adecuadas.} ``Objetiva'' no significa automática, infalible ni
...
\begin{description}[style=nextline]
    \item[Estímulo.]
    \verify{Tonos de frecuencia y nivel controlados, presentados por vía aérea
    mediante auriculares o por vía ósea mediante un vibrador.}

    \item[Magnitud registrada.]
    \verify{Nivel de audición del umbral para cada frecuencia, expresado en
    ...
    \verify{La vía aérea depende de la cadena completa desde el oído externo hasta la
    respuesta conductual. La vía ósea modifica la forma de estimular las
    cócleas, pero no constituye una medición aislada de una única estructura.}

    \item[Respuesta conductual.]
    \verify{Sí. La persona indica la detección del estímulo según una consigna
    y un procedimiento de búsqueda de umbral.}

    \item[Resultado básico esperado.]
    \verify{Un audiograma con umbrales por frecuencia, oído y vía. La
    simbología, las frecuencias examinadas y el procedimiento de
    enmascaramiento deben seguir normas audiométricas vigentes.}

    \item[Limitaciones generales.]
    \verify{Influyen la calibración, el ruido ambiente, la colocación del transductor,
...
\begin{figure}[htbp]
    \centering
    \input{figures/tikz/unidad-8/audiograma-conceptual.tex}
...
\begin{description}[style=nextline]
    \item[Estímulo.]
    \verify{Material verbal grabado o presentado de manera controlada, a
    niveles especificados por el procedimiento.}

    \item[Magnitud registrada.]
    \verify{Según la tarea, un nivel de reconocimiento expresado en
    \(\mathrm{dB\,HL}\), \(\mathrm{dB\,SL}\) o \(\mathrm{dB\,SPL}\), o un
    porcentaje de respuestas verbales correctas. La escala debe indicarse:
    esos tres niveles en decibeles no son equivalentes.}

    \item[Parte del sistema evaluada.]
    \verify{La respuesta integrada al habla, desde la recepción auditiva hasta
    los procesos lingüísticos y la respuesta solicitada.}

    \item[Respuesta conductual.]
    \verify{Sí. La persona señala o repite el material, de acuerdo con la
    prueba.}

    \item[Resultado básico esperado.]
    \verify{Puede obtenerse un umbral de recepción del habla y una función
    ...
    cruzado, con criterios dependientes del material y del protocolo.}

    \item[Limitaciones generales.]
    \verify{El idioma, la familiaridad léxica, la edad, la atención, la forma de
...
\verify{Una curva logoaudiométrica conceptual coloca el nivel de presentación, con su
...
material, del procedimiento y de la persona.}
...
\begin{figure}[htbp]
    \centering
    \input{figures/tikz/unidad-8/curva-logoaudiometrica-conceptual.tex}
...
\begin{description}[style=nextline]
    \item[Estímulo.]
    \verify{Un tono de sonda en el conducto auditivo mientras el equipo
    modifica la presión de aire en un intervalo controlado.}

    \item[Magnitud registrada.]
    \verify{Una magnitud de inmitancia acústica, habitualmente admitancia, en función
    ...
    \item[Parte del sistema evaluada.]
    \verify{La transferencia mecanoacústica dominada por el oído medio,
    condicionada también por el sellado de la sonda y el conducto.}

    \item[Respuesta conductual.]
    \verify{No requiere una respuesta voluntaria, aunque el movimiento, la
    vocalización o un sellado inadecuado pueden alterar el registro.}

    \item[Resultado básico esperado.]
    \verify{Un timpanograma: curva de admitancia o de otra magnitud de
    ...
    el tono de sonda y el protocolo.}

    \item[Limitaciones generales.]
    \verify{La curva no mide directamente la audición ni identifica de manera única
    una patología. Debe interpretarse junto con el volumen estimado del
    conducto, los reflejos cuando corresponda y otras pruebas.}
\end{description}
...
\begin{figure}[htbp]
    \centering
    \resizebox{\textwidth}{!}{%
        \input{figures/tikz/unidad-8/timpanogramas-esquematicos.tex}%
    }
...
\begin{description}[style=nextline]
    \item[Estímulo.]
    \verify{Tonos, ruidos de banda estrecha u otros sonidos ajustables en
    frecuencia y nivel.}

    \item[Magnitud registrada.]
    \verify{La frecuencia de correspondencia en \(\mathrm{Hz}\), el nivel de
    ...
    \item[Parte del sistema evaluada.]
    \verify{La comparación perceptual entre el tinnitus referido y estímulos
    externos; no mide una fuente acústica interna.}

    \item[Respuesta conductual.]
    \verify{Sí. La persona elige la alternativa que considera más parecida o
    informa cuándo se produce el efecto solicitado.}

    \item[Resultado básico esperado.]
    \verify{Una caracterización psicoacústica de correspondencia. Los
    protocolos pueden incluir frecuencia, nivel, nivel mínimo de
    enmascaramiento e inhibición residual, con variabilidad entre sesiones.}

    \item[Limitaciones generales.]
    \verify{El resultado depende de la consigna, del repertorio de estímulos y de la
...
\begin{description}[style=nextline]
    \item[Estímulo.]
    \verify{En las otoemisiones evocadas transitorias se emplean estímulos breves; en
    las otoemisiones por productos de distorsión se presentan dos tonos de
    frecuencias \(f_1\) y \(f_2\), expresadas en \(\mathrm{Hz}\).}

    \item[Magnitud registrada.]
    \verify{Presión sonora en el conducto auditivo mediante un micrófono de sonda. El
    equipo informa niveles de emisión y de ruido, en \(\mathrm{dB\,SPL}\), y
    relaciones señal--ruido en \(\mathrm{dB}\).}

    \item[Parte del sistema evaluada.]
    \verify{El registro se asocia principalmente con la función de las células
    ciliadas externas, pero depende también de que el estímulo y la emisión
    atraviesen adecuadamente el oído externo y el oído medio.}

    \item[Respuesta conductual.]
    \verify{No requiere una respuesta voluntaria.}

    \item[Resultado básico esperado.]
    \verify{Emisión presente o ausente según criterios del protocolo, o amplitud por
    banda de frecuencia. En programas de pesquisa puede mostrarse una salida
    operativa del tipo ``pasa'' o ``derivar'', que no equivale a diagnóstico.
    }

    \item[Limitaciones generales.]
    \verify{El ruido, el ajuste de la sonda y el estado del oído externo y medio pueden
    impedir el registro. Una emisión presente no prueba una audición normal y
    una emisión ausente no localiza por sí sola una lesión.}
\end{description}
...
\begin{description}[style=nextline]
    \item[Estímulo.]
    \verify{Clics, ráfagas tonales o estímulos de banda más controlada,
    presentados a niveles y tasas definidos.}

    \item[Magnitud registrada.]
    \verify{Diferencias de potencial eléctrico registradas con electrodos de
    ...
    \item[Parte del sistema evaluada.]
    \verify{Los potenciales auditivos de tronco cerebral reflejan actividad
    ...
    ventanas temporales y generadores diferentes.}

    \item[Respuesta conductual.]
    \verify{No requiere una respuesta voluntaria. Sí requiere controlar
    impedancias de electrodos, artefactos, estado de reposo y calidad del
    promedio.}

    \item[Resultado básico esperado.]
    \verify{Una forma de onda promediada con componentes identificables, latencias en
    \(\mathrm{ms}\) y amplitudes en \(\mathrm{\mu V}\), o una estimación
    de umbral según el protocolo.}

    \item[Limitaciones generales.]
    \verify{La respuesta depende del estímulo, el nivel, la tasa, el estado fisiológico
    y el ruido eléctrico. Una respuesta neural no mide comprensión del habla y
    una estimación electrofisiológica no es idéntica a un umbral conductual.}
\end{description}
...
\begin{description}[style=nextline]
    \item[Estímulo.]
    \verify{Clics o ráfagas tonales presentados por vía aérea con parámetros
    controlados.}

    \item[Magnitud registrada.]
    \verify{Potenciales eléctricos en microvoltios (\(\mathrm{\mu V}\)) y sus
    latencias en milisegundos (\(\mathrm{ms}\)).}

    \item[Parte del sistema evaluada.]
    \verify{La electrococleografía registra componentes asociados con la
    ...
    \item[Resultado básico esperado.]
    \verify{Formas de onda y relaciones entre componentes definidas por el
    protocolo, no un rótulo diagnóstico automático.}

    \item[Limitaciones generales.]
    \verify{El electrodo puede colocarse por vía extratimpánica o
    ...
    pregunta clínica y no son universales.}
\end{description}

\paragraph{OEA y potenciales: mediciones complementarias.}
\verify{La OEA registra una señal acústica en el conducto; un potencial evocado
registra una señal bioeléctrica mediante electrodos. Por lo tanto, una no
``confirma'' automáticamente a la otra.} Compararlas exige seguir la cadena
...
\begin{figure}[htbp]
    \centering
    \resizebox{\textwidth}{!}{%
        \input{figures/tikz/unidad-8/cadenas-oea-peat.tex}%
    }
...
alteración ni reproduce necesariamente la audición habitual. \verify{La
...
cada persona, dentro de un equipo profesional.}

\subsection{Audífonos}
\label{sec:u8-audifonos}

\verify{En su cadena funcional más simple, un micrófono convierte presión acústica en
...
frecuencia, del nivel de entrada y del tiempo.}
...
\verify{Los audífonos actuales pueden incluir compresión dependiente del
...
\verify{El implante coclear cambia la naturaleza de la señal dentro de la cadena. Un
...
electrodos entrega pulsos eléctricos en la cóclea.}

\verify{El objetivo es estimular poblaciones neurales auditivas evitando que
...
\verify{Un implante no es un audífono de mayor potencia: el audífono mantiene
una salida acústica, mientras que el implante produce estimulación eléctrica.}
\verify{Los criterios de candidatura y los resultados esperables cambian con
...
\verify{Estos dispositivos convierten una señal eléctrica en vibración mecánica
...
correcto decir que la energía viaja sólo a la cóclea del lado estimulado.}

\verify{Existen configuraciones externas, percutáneas y transcutáneas, con
...
evaluación individual y no se deduce de la etiqueta diagnóstica aislada.}

\subsection{Estimulación electroacústica}
\label{sec:u8-eas}

\verify{Este sistema combina una salida acústica y otra eléctrica en un mismo oído.}
\verify{Puede emplearse cuando existe audición
...
umbrales, del dispositivo, de la programación y de la evolución individual.}
...
\begin{figure}[htbp]
    \centering
    \resizebox{\textwidth}{!}{%
        \input{figures/tikz/unidad-8/cadenas-dispositivos.tex}%
    }
...
\verify{En la práctica profesional, la evaluación auditiva se construye con
...
habilitados y a protocolos vigentes.}
...
\begin{enumerate}[label=\textbf{C\arabic*.},leftmargin=*]
    ...
    \(L_{Aeq,2\,\mathrm{h}}=90\,\mathrm{dB(A)}\), tinnitus referido y un umbral
...
\begin{enumerate}[label=\textbf{NG\arabic*.},leftmargin=*]
    ...
    \item Una fuente aporta \(L_{Aeq,2\,\mathrm{h}}=92\,\mathrm{dB(A)}\)
    ...
    \begin{enumerate}
        ...
        \item Exprese \(L_{Aeq,8\,\mathrm{h}}\) con una cifra decimal.
...
\begin{enumerate}[label=\textbf{NA\arabic*.},leftmargin=*]
    ...
    \item Una fuente aporta \(L_{Aeq,0{,}5\,\mathrm{h}}=96\,\mathrm{dB(A)}\)
    durante \(0{,}5\,\mathrm{h}\), y su contribución durante las otras
    \(7{,}5\,\mathrm{h}\) es despreciable. Calcule
    \(L_{Aeq,8\,\mathrm{h}}\) y redondee el resultado a una cifra decimal.
...
\begin{enumerate}[label=\textbf{INT\arabic*.},leftmargin=*]
    ...
    \begin{itemize}
        ...
        \(L_{Aeq,1\,\mathrm{h}}=94\,\mathrm{dB(A)}\), con contribución
    ...
    \begin{enumerate}
        ...
        \item Calcule \(L_{Aeq,8\,\mathrm{h}}\) con una cifra decimal.
...
\begin{enumerate}[label=\textbf{C\arabic*.},leftmargin=*]
    ...
    \item \(L_{Aeq,2\,\mathrm{h}}=90\,\mathrm{dB(A)}\) describe la exposición,
...
\section{Fuentes pendientes de incorporación}
\label{sec:u8-fuentes}

Las afirmaciones clínicas, fisiológicas, normativas y de desempeño que
requieren respaldo externo están marcadas con \verb|\verify{...}|. Antes de
cerrar la revisión bibliográfica deben vincularse con fuentes primarias,
normas técnicas vigentes, documentación oficial de salud y textos académicos
reconocidos. En particular, quedan pendientes la normativa argentina sobre
exposición ocupacional a ruido, los procedimientos audiométricos actuales,
los protocolos de pesquisa y las guías vigentes para evaluación y selección
de dispositivos.
