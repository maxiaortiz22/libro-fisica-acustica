...
la actividad neural se desarrollarán en la Unidad 7, mientras que las
alteraciones, los estudios auditivos y la rehabilitación corresponden a la
Unidad 8.
...
según el capítulo~\ref{chap:unidad-2}; oscilación y onda viajera según el
capítulo~\ref{chap:unidad-3}; y presión acústica, velocidad de partícula,
impedancia y nivel de presión sonora según el
capítulo~\ref{chap:unidad-4}. El capítulo~\ref{chap:unidad-5} aporta la
...
\begin{figure}[htbp]
    \centering
    \input{figures/tikz/unidad-6/cadena-transduccion.tex}
...
\verify{La geometría del pabellón y de la cabeza produce una respuesta
...
espectrales varían entre personas y aportan pistas para la localización.}
...
\verify{El CAE humano es curvo, posee sección variable y termina en una
...
transferencia dependiente de la frecuencia y de la posición de medición.}
...
\verify{La membrana timpánica real es curva, no se desplaza como un pistón
...
sistema osicular.} Por lo tanto, la ecuación~\eqref{eq:u6-fuerza-timpano}
...
\verify{El oído medio es una cavidad aérea que contiene la cadena formada por
...
otra frontera móvil entre el oído medio y la cóclea.}

\verify{La trompa de Eustaquio conecta funcionalmente la cavidad del oído
medio con la nasofaringe. Su apertura intermitente contribuye a equilibrar
presiones, ventilar y drenar el oído medio.} Si la presión estática a ambos
...
\begin{figure}[htbp]
    \centering
    \input{figures/tikz/unidad-6/organizacion-oido-periferico.tex}
...
\begin{figure}[htbp]
    \centering
    \input{figures/tikz/unidad-6/adaptacion-oido-medio.tex}
...
\verify{Ante determinados estímulos sonoros de nivel elevado, una vía refleja
...
disminuye cuando aumenta el nivel por encima del umbral.}
...
\verify{El cambio de inmitancia asociado con el reflejo puede medirse con
...
una etiología.} La interpretación diagnóstica y los protocolos de medición se
reservan para la Unidad 8.
...
\verify{En una descripción funcional pueden distinguirse al menos las
...
\begin{enumerate}
    \item \verify{\textbf{Radiación hacia el CAE:} la vibración de sus paredes
    produce presión acústica en el aire del canal; la oclusión puede modificar
    esta contribución.}
    \item \verify{\textbf{Inercia de los huesecillos:} el movimiento relativo
    entre la cadena osicular y las paredes de la cavidad puede desplazar el
    estribo.}
    \item \verify{\textbf{Inercia de los fluidos cocleares:} el movimiento del
    cráneo puede producir movimiento relativo de los fluidos respecto de la
    cápsula.}
    \item \verify{\textbf{Deformación de la cápsula coclear:} cambios pequeños
    de forma pueden generar diferencias de presión entre compartimentos.}
    \item \verify{\textbf{Transmisión por tejidos y cavidades:} otras rutas,
...
\begin{figure}[htbp]
    \centering
    \input{figures/tikz/unidad-6/conduccion-osea-multimecanismo.tex}
...
\verify{La cóclea contiene tres compartimentos longitudinales: rampa
...
mediante el helicotrema; la rampa media contiene endolinfa.}

\verify{La membrana de Reissner separa la rampa vestibular de la rampa media.
...
haces de estereocilios del órgano de Corti y participa en su micromecánica.}
...
\begin{figure}[htbp]
    \centering
    \input{figures/tikz/unidad-6/arquitectura-coclear.tex}
...
\verify{Una estimulación tonal produce una onda viajera en la partición
...
frecuencias bajas, en regiones más apicales.}
...
\verify{La variación longitudinal de rigidez, dimensiones y carga de la
...
asociados con las células ciliadas externas.}

\subsection{Señales débiles e intensas}
\label{sec:u6-debiles-intensas}

\verify{Para señales débiles, el proceso activo coclear aumenta localmente la
...
de sonoridad requiere procesamiento adicional y se estudiará en la Unidad 7.
...
\begin{figure}[htbp]
    \centering
    \input{figures/tikz/unidad-6/onda-viajera-tonotopia.tex}
...
\verify{El órgano de Corti se apoya sobre la membrana basilar y contiene
...
\verify{\textbf{Células ciliadas internas.} Las CCI constituyen la vía
...
aferentes del ganglio espiral.}

\verify{\textbf{Células ciliadas externas.} Las CCE cambian su longitud en
...
frecuencial, especialmente para señales débiles.}
...
\verify{Los estereocilios se organizan en haces polarizados y se conectan
...
reduce. Los enlaces no son los canales.}

\verify{La endolinfa posee una composición iónica y un potencial eléctrico
...
único que funcione como umbral mecánico de todo o nada.}

\verify{En una CCI, la despolarización abre canales de calcio
...
\begin{figure}[!htbp]
    \centering
    \input{figures/tikz/unidad-6/funciones-cci-cce.tex}
...
\verify{La frecuencia se representa inicialmente mediante mecanismos
...
reducirse a una cifra universal.}
...
\verify{Al aumentar el nivel de una señal, cambian el movimiento coclear, el
...
\verify{La relación entre una estructura, una prueba y una alteración clínica
no es unívoca. La interpretación exige protocolos, calibración, antecedentes
y pruebas cruzadas.} Estas condiciones se desarrollarán en la Unidad 8.
