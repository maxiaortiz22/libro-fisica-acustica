...
\verify{Las denominaciones, tolerancias y usos audiométricos de los ruidos con
...
vigente debe acompañar cualquier especificación cuantitativa.}
...
\verify{En audiometría tonal pueden emplearse ruidos de banda estrecha como
señales enmascarantes, pero sus bandas efectivas, calibración y presentación
deben corresponder al equipo, al transductor y al procedimiento vigente.}
...
\begin{figure}[htbp]
    \centering
    \input{figures/tikz/unidad-10/conformacion-espectral.tex}
...
\verify{La normalización de una exposición a una jornada de referencia, el
...
ediciones o jurisdicciones diferentes.}
...
\verify{En audiometría, el enmascaramiento puede utilizarse para reducir la
...
audiológico vigente y no se deducen solo del tipo de ruido.}
...
\begin{figure}[htbp]
    \centering
    \input{figures/tikz/unidad-10/enmascaramiento-audiometrico-conceptual.tex}
...
\verify{La acufenometría puede incluir la comparación de tonos o ruidos con la
...
no prescribe un tipo de ruido ni un nivel para tinnitus.}
...
\verify{La exposición a determinados niveles y duraciones puede asociarse con
...
por sí sola un diagnóstico ni una relación causal individual.}

\verify{El ruido puede interferir con la comunicación oral y con tareas que
...
características del habla, la tarea y el oyente.}
...
\verify{Antes de aplicar recomendaciones de exposición o seleccionar
...
educativos o clínicos.}

\verify{Los límites de ruido admisible para salas de prueba audiométrica
...
garantiza por sí solo que una cabina sea adecuada.} Véase también la
...
\verify{Las jerarquías de control ocupacional suelen priorizar la eliminación
...
verificarse en el marco normativo vigente.}
...
\begin{figure}[htbp]
    \centering
    \input{figures/tikz/unidad-10/control-fuente-trayecto-receptor.tex}
...
\begin{description}
    ...
    \item[\textbf{Protección auditiva:}] \verify{dispositivo y programa
...
\begin{enumerate}[label=\textbf{G\arabic*.},leftmargin=*]
    ...
    \[
        L_{Aeq,1}=70\,\mathrm{dB(A)}
        \qquad\text{y}\qquad
        L_{Aeq,2}=80\,\mathrm{dB(A)}.
    \]
    \begin{enumerate}
...
\begin{enumerate}[label=\textbf{A\arabic*.},leftmargin=*]
    ...
    \(L_{Aeq}=74\,\mathrm{dB(A)}\),
    \(78\,\mathrm{dB(A)}\) y \(82\,\mathrm{dB(A)}\). Calcule
    \(L_{Aeq,30\,\mathrm{min}}\), expresado en \(\mathrm{dB(A)}\), y compárelo
...
\begin{enumerate}[label=\textbf{D\arabic*.},leftmargin=*]
    ...
    \item ``Si dos señales tienen el mismo \(L_{Aeq,T}\), producen los mismos
...
\begin{enumerate}[label=\textbf{D\arabic*.},leftmargin=*]
    ...
    \item Es incorrecta. Igual \(L_{Aeq,T}\) no implica iguales máximos,
...
\section*{Fuentes externas pendientes}

Los bloques marcados con \verb|\verify{...}| requieren validación antes de
considerar cerrada la unidad. Deben utilizarse, según el caso:
\begin{itemize}
