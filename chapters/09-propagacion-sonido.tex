...
oyente y el contexto, como se estudió en el Capítulo~\ref{chap:unidad-7}.
...
\verify{La directividad de una fuente real depende de la frecuencia, de su
geometría, de su montaje y de las superficies próximas. Un único valor de
\(Q\) no describe por completo una fuente de banda ancha.}
...
\verify{En una situación real, el nivel recibido depende de la emisión de la
...
transmisión y la posición del receptor.} La ecuación~\eqref{eq:u9-distancia}
...
modelo ya presentado en la Unidad 2 y no debe extrapolarse fuera del intervalo
en el que se valide.
...
\verify{Cuando \(c\) cambia con la altura, un modelo de rayos acústicos predice
...
curvatura descendente y favorecer trayectos hacia receptores distantes.}
...
\begin{figure}[htbp]
    \centering
    \input{figures/tikz/unidad-9/gradientes-termicos.tex}
...
\(\text{m}/\text{s}\). \verify{Un viento aproximadamente uniforme modifica
principalmente el tiempo de llegada respecto del suelo; no justifica por sí
solo una regla universal de aumento o disminución del nivel de presión sonora.}

\verify{Cuando la componente del viento cambia con la altura, el gradiente de
...
\begin{figure}[htbp]
    \centering
    \input{figures/tikz/unidad-9/gradiente-viento.tex}
...
\verify{Para un gas ideal bajo un modelo adiabático, la velocidad puede
escribirse como}
...
\verify{La altitud puede modificar presión, densidad, humedad y temperatura,
...
salida de auriculares audiométricos sin datos de calibración y del fabricante.}
...
\verify{La atenuación atmosférica depende de la frecuencia, la temperatura,
la humedad, la presión y la distancia. En general, no puede representarse
mediante una única corrección independiente de la frecuencia.} Si
...
\verify{En un balance energético ideal, sin otras rutas, estas fracciones
cumplen}
...
\begin{figure}[htbp]
    \centering
    \input{figures/tikz/unidad-9/balance-superficie.tex}
...
\verify{Para un recinto que se aproxima a las hipótesis de campo difuso y
...
\verify{Los coeficientes y el tiempo de reverberación deben considerarse por
bandas de frecuencia. La fórmula de Sabine puede perder precisión en recintos
muy absorbentes, pequeños o con absorción distribuida de manera muy desigual.}
...
\verify{Para rayos que atraviesan una interfaz ideal entre dos medios
...
intervenir distintos tipos de onda y conversión modal.} Por este motivo, la
...
\verify{La difracción suele ser más significativa cuando esas dimensiones son
comparables con \(\lambda\) o menores. Cuando el obstáculo es grande respecto
de \(\lambda\), puede formarse una región de sombra más marcada.} Esto explica
...
\begin{figure}[htbp]
    \centering
    \input{figures/tikz/unidad-9/difraccion-barrera.tex}
...
\verify{La pérdida por transmisión \(TL\), o el índice de reducción sonora
\(R\) según el contexto de medición, puede relacionarse con \(\tau\) mediante}
...
medidos en cualquier par de puntos. \verify{Los procedimientos de laboratorio
...
crear rutas laterales que reduzcan el aislamiento efectivo.}
...
\verify{Una forma didáctica habitual de la ley de masas, con \(m\) expresada
en \(\text{kg}/\text{m}^2\) y \(f\) en \(\text{Hz}\), es}
...
\verify{La ecuación~\eqref{eq:u9-ley-masas} no describe por sí sola una pared
...
la fuente adoptada antes de utilizar valores absolutos.}
...
\begin{figure}[!t]
    \centering
    \input{figures/tikz/unidad-9/ley-masas-pared-simple.tex}
...
\verify{En un proyecto real, acondicionamiento y aislamiento pueden ser
...
absorbente. \verify{Su desempeño depende del conjunto formado por paredes,
...
limitar el resultado aun cuando los paneles posean una reducción sonora alta.}
...
\begin{figure}[htbp]
    \centering
    \input{figures/tikz/unidad-9/cabina-sistema.tex}
...
\(\text{dB(A)}\). \verify{Los niveles máximos admisibles para una audiometría
...
\verify{La aptitud de una cabina debe demostrarse mediante mediciones por
...
\verify{Los procedimientos audiométricos, la aceptación normativa de una
cabina y el diseño constructivo requieren fuentes vigentes, instrumentación
adecuada y personal competente. Esta unidad no reemplaza esos procedimientos.}
...
Las afirmaciones atmosféricas, constructivas, audiométricas y normativas que
requieren respaldo externo están marcadas con \verb|\verify{...}|. Antes de
cerrar la revisión bibliográfica deben vincularse con fuentes académicas
primarias, textos reconocidos, normas técnicas vigentes y documentación
oficial.

\verify{Para el ruido ambiente de audiometría tonal deben revisarse la edición
...
correspondiente.} No se incorporan todavía tablas ni valores normativos porque
...
medirse y verificarse. La Unidad 10 utilizará estas distinciones para estudiar
