...
El capítulo~\ref{chap:unidad-3} introdujo período, frecuencia, fase,
sinusoides y superposición. El capítulo~\ref{chap:unidad-4} definió presión
...
se estudió en el capítulo~\ref{chap:unidad-4}.
...
\begin{figure}[htbp]
    \centering
    \input{figures/tikz/unidad-5/espectro-respuesta-sistema.tex}
...
de la fuente. \verify{La interpretación de formantes y otros parámetros de voz
requiere definir la vocal, el hablante, el método de análisis y el alcance
clínico, además de incorporar bibliografía especializada.}
...
\begin{figure}[htbp]
    \centering
    \input{figures/tikz/unidad-5/componentes-espectrales.tex}
...
\verify{Se usa de manera convencional \(20\,\text{Hz}\) como frontera entre
...
académica que declare las condiciones de escucha.}
...
sea necesariamente imperceptible: \verify{la detección puede depender del
nivel, la duración, la distorsión del sistema y la presencia de vibración.}
...
\verify{Los valores numéricos de rangos vocales, instrumentales y auditivos
deben incorporarse solo después de seleccionar fuentes, condiciones de
medición y población de referencia.}
...
\(b=3\) y \(f_H/f_L=2^{1/3}\). \verify{Los instrumentos normalizados usan
series de frecuencias centrales nominales y requisitos de tolerancia que deben
documentarse con la edición aplicable de la norma de filtros de banda.}
...
\begin{figure}[htbp]
    \centering
    \input{figures/tikz/unidad-5/bandas-octava-tercio.tex}
...
\verify{Las respuestas exactas, tolerancias, intervalo de frecuencias y
denominaciones de A, C y Z deben verificarse contra la edición aplicable de la
norma de sonómetros.}
...
\verify{Como dato nominal pendiente de incorporar con su fuente primaria, se
adopta \(A(63\,\text{Hz})=-26{,}2\,\text{dB}\).} Si un tono de
...
% TODO(figure,blocked-by-verify): generar las curvas A, C y Z solo después de
% verificar sus ecuaciones, tolerancias y edición normativa. Una curva
% aproximada podría confundirse con la respuesta nominal de un instrumento.
...
\begin{figure}[htbp]
    \centering
    \input{figures/tikz/unidad-5/cadena-sonometro.tex}
...
Sea \(p_F(t)\) la presión acústica después de aplicar una ponderación
frecuencial \(F\), medida en pascales, durante un intervalo \(T\), medido en
segundos. El nivel equivalente es
...
\begin{equation}
    ...
    \left[
        \frac{1}{T}
        \int_0^T
        \frac{p_F^2(t)}{p_\mathrm{ref}^{\,2}}\,dt
    \right],
    \label{eq:u5-leq}
\end{equation}
...
exponencial. \verify{En sonometría normalizada, las constantes nominales
asociadas con F (\emph{Fast}) y S (\emph{Slow}) son
\(0{,}125\,\text{s}\) y \(1\,\text{s}\), respectivamente; su definición
exacta y tolerancias deben documentarse con la norma aplicable.}

Un nivel máximo \(L_{F\max}\) es el mayor valor alcanzado por un nivel con una
ponderación frecuencial y temporal especificadas. Un nivel de pico se obtiene
mediante un detector de pico y no debe confundirse con \(L_{F\max}\) ni con
una indicación denominada \emph{Impulse}.

\subsection{Verificación del instrumento y ruido de fondo}
\label{sec:u5-verificacion-sonometro}

La trazabilidad metrológica, la calibración periódica y la comprobación de
campo no son sinónimos. \verify{El procedimiento, el calibrador, la frecuencia,
la tolerancia y la periodicidad deben tomarse de normas y documentación
oficiales aplicables al instrumento y a la medición.}

En una sala destinada a audiometría no debe emplearse un único límite universal
en dB(A). \verify{Los niveles máximos admisibles de ruido ambiente dependen de
...
audiométrica y la jurisdicción adoptadas.} El desarrollo arquitectónico de
...
\verify{Toda asociación entre parámetros espectrales de voz, hallazgos
auditivos, ajuste de dispositivos y decisiones clínicas debe sustentarse en
fuentes especializadas y delimitar qué mide cada procedimiento.}
...
\begin{description}[style=nextline]
    ...
    \item[\textbf{Nivel equivalente (\(L_{F\mathrm{eq},T}\)):}]
